...
function MemberCard({ member }: { member: SanityLifeMember }) {
  ...
  return (
    <Reveal className="flex flex-col items-center rounded-2xl border border-faint bg-card p-[30px] text-center shadow-sm transition-all duration-300 hover:-translate-y-1.5 hover:shadow-md">
      <div className="relative mb-5 h-[110px] w-[110px] overflow-hidden rounded-full ring-2 ring-red/20">
        {member.photoUrl ? (
          <Image
            src={member.photoUrl}
            alt={member.name}
          ...
          <div className="flex h-full w-full items-center justify-center bg-bg-soft">
            <span className="font-serif text-[32px] font-bold text-red">{initials}</span>
          </div>
        )}
      </div>

      <h3 className="mb-1.5 font-serif text-[18px] font-bold leading-snug text-ink">{member.name}</h3>

      {member.memberSince && (
        <p className="mb-3 text-[11px] font-semibold uppercase tracking-[1px] text-red">
      ...
      {/* lifetime badge */}
      <span className="mt-4 inline-flex items-center gap-1.5 rounded-full border border-red/25 bg-red/10 px-3 py-1 text-[10px] font-semibold uppercase tracking-[1.5px] text-red">
      ...
      <main>
        ...
        <section className="px-[6%] pb-[100px] pt-10">
          <Reveal className="mb-10">
            <p className="block text-xs font-semibold tracking-[3px] text-red">
