          ...
          <Reveal className="rounded-[10px] border border-white/5 bg-card-2 p-[40px] max-[560px]:p-[24px]">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-5" encType="multipart/form-data">
              ...
              {isMembership && (
                <>
                  <div>
                    <label htmlFor="phone" className="sr-only">{t.form.phonePlaceholder}</label>
                    <input type="tel" id="phone" name="phone" placeholder={t.form.phonePlaceholder} required className={inputClass} />
                  </div>

                  <div className="rounded-[8px] border border-white/5 bg-card p-5 space-y-4">
                    <h3 className="font-serif text-[16px] font-semibold text-white">{t.form.idVerification.heading}</h3>
                    <p className="text-[13px] text-muted">{t.form.idVerification.desc}</p>
                    <div>
                      <label htmlFor="state_id" className="mb-2 block text-[13px] font-medium text-white">{t.form.idVerification.stateIdLabel}</label>
                      <input type="file" id="state_id" name="state_id_photo" accept="image/png, image/jpeg, image/webp, application/pdf" required
                    ...
                    <div>
                      <label htmlFor="citizenship_photo" className="mb-2 block text-[13px] font-medium text-white">{t.form.idVerification.citizenshipLabel}</label>
                      <input type="file" id="citizenship_photo" name="citizenship_photo" accept="image/png, image/jpeg, image/webp, application/pdf" required
                        className={`${inputClass} file:mr-4 file:rounded-[4px] file:border-0 file:bg-red-soft file:px-4 file:py-2 file:text-sm file:font-semibold file:text-white hover:file:bg-red-soft/90 cursor-pointer p-2`} />
                    </div>
                  </div>

                  <div className="rounded-[8px] border border-red-soft/30 bg-red-soft/5 p-5">
                    <h3 className="mb-2 font-serif text-[18px] font-semibold text-white">{t.form.payment.heading}</h3>
                    <p className="text-[14px] text-muted mb-4">
                      {t.form.payment.desc} <strong>{inquiryType}</strong>.
                    </p>
                    <div className="mx-auto mb-4 flex h-[150px] w-[150px] items-center justify-center overflow-hidden rounded-[8px] bg-white">
                      <Image src="/path-to-your-qr-code.png" alt="Payment QR Code" width={150} height={150} className="h-full w-full object-cover" />
                    </div>
                    <div>
                      <label htmlFor="screenshot" className="mb-2 block text-[14px] font-medium text-white">{t.form.payment.screenshotLabel}</label>
                      <input type="file" id="screenshot" name="payment_screenshot" accept="image/png, image/jpeg, image/webp" required
                        className={`${inputClass} file:mr-4 file:rounded-[4px] file:border-0 file:bg-red-soft file:px-4 file:py-2 file:text-sm file:font-semibold file:text-white hover:file:bg-red-soft/90 cursor-pointer p-2`} />
                    </div>
                  </div>
                </>
              ...
              <div>
                ...
                <textarea
                  ...
                  name="message"
                  rows={4}
                  placeholder={isMembership ? t.form.messagePlaceholderMembership : t.form.messagePlaceholderInquiry}
              ...
              >
                {isSubmitting ? t.form.sending : (isMembership ? t.form.submitMembership : t.form.submitInquiry)}
              </button>
